...
export const realTimetable = async (untis: any, date: Date, useShortSubjectName: Boolean): Promise<any> => {
	...
	timegrid.forEach((tu: any, i: number) => {
		...
		const lessonsThisTU = timetable.filter((lesson: any) => lesson.startTime === tu.startTime && lesson.endTime === tu.endTime);
...
export const schoolyear = async (untis: any): Promise<any> => untis.getCurrentSchoolyear();
export const exams = async (untis: any, start: Date, end: Date): Promise<any> => untis.getExamsForRange(start, end);

// Simple factory for Untis instance (avoid hardcoding credentials in source for production)
export const createUntis = (school: string, user: string, password: string, host: string) => new WebUntis(school, user, password, host);
